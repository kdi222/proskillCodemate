...
function overallReport($studentid) {
    global $DB;
    ...
    if ($courses) {
        foreach ($courses as $course) {
            ...
            foreach ($mockgrades as $mockgrade) {
             ...
             // Retrieve all students' total grades for ranking
             $all_students = get_all_students(); // You need to implement this function to retrieve all students' IDs
             $student_totals = [];

             foreach ($all_students as $id) {
                 $student_totals[$id] = calculate_total_grades($id); // You need to implement this function to calculate total grades for each student
             }

             // Sort students by their total grades in descending order
             arsort($student_totals);

             // Determine the rank of the current student
             $rank = array_search($studentid, array_keys($student_totals)) + 1;

             // Add the rank to the report
             $report['student_rank'] = $rank;
...
function calculate_total_grades($studentid) {
    global $DB;
    ...
    if ($courses) {
        foreach ($courses as $course) {
            ...
            foreach ($mockgrades as $mockgrade) {
                $total_mock_grade = $mockgrade->techmark + $mockgrade->communicationmark + $mockgrade->practicalmark;
                $total_grades += $total_mock_grade;
            }
        }
    }
...
function get_all_students() {
    ...
    // This is a placeholder function, implement logic to retrieve all students
    $students = $DB->get_records_sql("SELECT id FROM {user} WHERE deleted = 0 AND suspended = 0");

    return array_keys($students);
}

function get_student_group($userid) {
    global $DB;

    // Define the SQL query.
    $sql = "SELECT pp.fullname 
            FROM {enrol_programs_allocations} pa 
            JOIN {enrol_programs_programs} pp 
            ON pp.id = pa.programid 
            WHERE pa.userid = :userid 
            ORDER BY pa.programid DESC 
            LIMIT 1";

    // Execute the query and fetch the result.
    $result = $DB->get_record_sql($sql, array('userid' => $userid));

    // Check if we have a result and return the fullname.
    if ($result) {

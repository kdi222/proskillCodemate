...
$PAGE -> set_title(get_string('listnotstartedcourses', 'block_myprogress', ($user->firstname.' '.$user->lastname)));
$PAGE -> set_heading(get_string('listnotstartedcourses', 'block_myprogress', ($user->firstname.' '.$user->lastname)));
...
$PAGE -> navbar -> add(get_string('listnotstartedcourses', 'block_myprogress', ($user->firstname.' '.$user->lastname)));
echo $OUTPUT -> header();
echo $OUTPUT -> heading(get_string('listnotstartedcourses', 'block_myprogress', ($user->firstname.' '.$user->lastname)));
...
foreach ($courses as $course) {
	...
	/**
	* Get enrollment date if not course completion traking enabled
	* @author Esteban E. 
	* @since June 28 2017
	* @paradiso
	*/
...
echo $OUTPUT->render_from_template('block_myprogress/list_notstarted', $data);
echo $OUTPUT -> footer();